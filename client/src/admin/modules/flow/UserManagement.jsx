...
const UserManagement = () => {
  ...
  return (
    <div className="space-y-6">
      {selectedUserId ? (
        // ==========================================
        // ≡ƒîƒ A TO Z USER DEEP-DIVE DASHBOARD VIEW
        // ==========================================
        <motion.div
                      ...
                      {userDetails?.payments?.length > 0 && (
                        <div className="text-right">
                          ...
                          <span className="text-lg font-black text-white">
                            Γé╣{userDetails.payments.reduce((acc, curr) => curr.status === 'captured' ? acc + (curr.amount / 100) : acc, 0).toLocaleString()}
                          </span>
                        </div>
                      )}
                    </div>
                        ...
                        <tbody className="divide-y divide-white/5 text-xs text-gray-300">
                          {userDetails?.payments?.map(payment => (
                            <tr key={payment._id} className="hover:bg-white/[0.01] transition-colors">
                              ...
                              <td className="py-4 font-extrabold text-white">Γé╣{payment.amount / 100}</td>
        ...
        // ==========================================
        // ≡ƒôï STANDARD USERS LIST VIEW TABLE
        // ==========================================
        <>
          {/* Search and Filter */}
