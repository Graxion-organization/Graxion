  ...
  return (
    <div className="pb-20">
      ...
      >
        ...
        ) : activeSection?.isCustom && activeSection.id === "sidebar" ? (
          <>
            <div className="p-6 border-b border-white/10 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white/[0.02]">
              <div className="flex items-center gap-3">
                <activeSection.icon className="w-6 h-6 text-emerald-500" />
                <div>
                  <h2 className="text-xl font-bold">{activeSection.title}</h2>
                  <p className="text-sm text-gray-400 mt-1">Enable or disable sidebar sections globally for non-admin users.</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {[
                  "Home", "Analytics", "Inbox", "Automations", "Broadcast", 
                  "Contacts", "Social Hub", "Integrations", "Agent Studio", "Settings"
                ].map(label => {
                  const settingObj = getSetting("sidebar_settings") || { value: {} };
                  const currentVal = settingObj.value?.[label] !== false; // default true
                  ...
                  return (
                    ...
                    >
                      ...
                      <button
                        ...
                        disabled={saving}
                        className={`relative inline-flex h-7 w-12 items-center rounded-full transition-colors focus:outline-none ${
                          currentVal ? "bg-emerald-500" : "bg-gray-700"
                        }`}
                      >
                        <span
                          className={`inline-block h-5 w-5 transform rounded-full bg-white transition-transform ${
