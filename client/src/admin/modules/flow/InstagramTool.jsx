...
export default function InstagramTool() {
  ...
  const fetchAccounts = async () => {
    ...
    try {
      ...
      const res = await axios.get(`${baseURL}/admin/Camera/accounts`, {
        withCredentials: true
      });
      setAccounts(res.data.data.accounts || []);
    } catch (err) {
      toast.error('Failed to load Camera accounts');
    } finally {
  ...
  const fetchMedia = async (account) => {
    ...
    try {
      ...
      axios.get(`${baseURL}/admin/Camera/${account._id}/stats`, {
        withCredentials: true
      }).then(res => {
      ...
      const res = await axios.get(`${baseURL}/admin/Camera/${account._id}/media`, {
        withCredentials: true
      });
  ...
  const fetchComments = async (media) => {
    ...
    try {
      // JWT via cookies
      const baseURL = process.env.REACT_APP_API_URL || 'http://localhost:5000/api';
      const res = await axios.get(`${baseURL}/admin/Camera/${selectedAccount._id}/media/${media.id}/comments`, {
        withCredentials: true
      });
  ...
  const handleSendComment = async () => {
    ...
    try {
      ...
      await axios.post(`${baseURL}/admin/Camera/comment`, {
            ...
            <button 
              onClick={async () => {
                try {
                  // JWT via cookies
                  const baseURL = process.env.REACT_APP_API_URL || 'http://localhost:5000/api';
                  await axios.post(`${baseURL}/admin/Camera/trigger-worker`, {}, {
                    withCredentials: true
                  });
            ...
            <button
              onClick={async () => {
                try {
                  // JWT via cookies
                  const baseURL = process.env.REACT_APP_API_URL || 'http://localhost:5000/api';
                  await axios.post(`${baseURL}/admin/Camera/auto-reply-post`, {
                    accountId: selectedAccount._id,
